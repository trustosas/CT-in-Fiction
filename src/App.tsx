...
import { Search, ArrowRight, X, Zap, Activity, Filter, Compass, Layers, ChevronLeft, ChevronRight, ChevronDown, Info, Loader2, AlertCircle, AlertTriangle, Wrench, Menu, Check, User, FileText, Hash, Settings as SettingsIcon, Sun, Moon, Laptop } from 'lucide-react';
...
const preloadImages = async (characters: Character[], forceBust = false) => {
  ...
  if (forceBust && 'caches' in window) {
    try {
      ...
      await caches.delete('ct-image-cache-v4');
      console.log('[ImagePreloader] Wiped image caches on manual sync');
    } catch (e) {
...
function MaintenanceBanner() {
  ...
  if (isDismissed) {
    return (
      <div className="bg-red-700 text-white text-[11px] font-mono px-4 py-1.5 flex items-center justify-between border-b border-red-800 shadow-sm z-[90]">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 px-1.5 py-0.5 rounded bg-red-950/80 text-red-200 font-bold uppercase tracking-wider text-[9px]">
            <span className="w-1.5 h-1.5 rounded-full bg-red-300 animate-pulse" />
            Maintenance
          </span>
          <span className="opacity-90">Maintenance mode active.</span>
        </div>
        <button 
          onClick={() => setIsDismissed(false)}
          className="underline hover:opacity-80 transition-opacity uppercase tracking-wider text-[10px] cursor-pointer"
        >
          Show Banner
        </button>
      </div>
  ...
  return (
    <motion.div
      ...
      className="relative z-[90] bg-gradient-to-r from-red-600 via-rose-600 to-red-700 text-white shadow-md border-b border-red-800/80"
    >
      <div className="max-w-[2000px] mx-auto px-4 sm:px-6 py-2.5 md:px-12 lg:px-24 flex items-center justify-between gap-3 sm:gap-4 flex-wrap sm:flex-nowrap">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <div className="p-1.5 bg-red-800/80 rounded-full shrink-0 border border-red-400/40 flex items-center justify-center shadow-inner">
            <AlertTriangle className="w-4 h-4 text-white animate-pulse" />
          </div>
          <div className="flex items-center gap-2.5 flex-wrap min-w-0">
            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-black/30 border border-white/25 text-white font-mono font-bold text-[10px] sm:text-[11px] uppercase tracking-widest shrink-0">
              <span className="w-2 h-2 rounded-full bg-amber-300 animate-ping" />
              Maintenance Mode
            </span>
            <p className="text-xs sm:text-sm font-medium text-white/95 truncate sm:text-balance">
              Scheduled maintenance is currently in progress. Some features or data syncs may be temporarily limited.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0 ml-auto sm:ml-0">
          <button
            ...
            className="text-[10px] sm:text-xs font-mono uppercase tracking-wider bg-black/20 hover:bg-black/35 px-2.5 py-1 rounded transition-colors flex items-center gap-1 text-white border border-white/20 cursor-pointer"
          ...
          <button
            ...
            className="p-1 hover:bg-black/20 rounded-full transition-colors text-white/80 hover:text-white cursor-pointer"
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="border-t border-white/15 bg-black/25 px-4 sm:px-6 py-3 md:px-12 lg:px-24 text-xs font-sans text-white/90 space-y-1.5"
          >
            <div className="max-w-4xl font-mono text-[11px] leading-relaxed space-y-1">
              <p>• System maintenance and database optimizations are actively in progress.</p>
              <p>• Live synchronization with external datasources and GitHub markdown fetches may experience intermittent delays.</p>
              <p className="text-white/70 text-[10px]">All saved user configurations and theme preferences remain intact.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
